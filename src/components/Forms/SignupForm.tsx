...
export function SignupForm() {
  ...
  const handleSignup = async (e: React.SyntheticEvent): Promise<void> => {
    ...
    try {
      ...
      const register = webAuthn.useWebAuthnRegistration(newUser.data?.addUser?.user._id, token);
      ...
      newUser?.data?.addUser?.user &&
        (await deleteUser({variables: {_id: newUser.data?.addUser?.user._id}}).catch(err =>
          console.log('Error deleting user', err),
        ));

      newUser && localStorage.removeItem('trash-user');
      const message =
        (error['message'] as string) || 'Something went wrong. Please try again later.';
      const messageToDisplay = message.toLowerCase().includes('could not verify the attestation')
        ? 'Select an authentication method that supports user verification. Select the Use a phone, tablet, or security key option'
        : message;
  ...
  return isMounted ? (
    <form className="Form-base">
      ...
      <p className="Webauth-notice">
        TrashScanner&trade; uses WebAuthn/FIDO2 Security tokens to protect your account. You can
        learn more at{' '}
        <span>
          <a
            target="_blank"
