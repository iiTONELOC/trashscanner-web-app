...
const createLoginOptionsURL = (forUsername: string) =>
  host + `/webauthn/assertion/options/${forUsername}`;
const createLoginResultURL = (forUserId: string) =>
  host + `/webauthn/assertion/result/${forUserId}`;

// REST API
// Registration Steps
// 1. Create options
export const useFetchWebAuthnOptions = (forUserId: string, withAuthToken: string) => {
  const webAuthnOptionURL = createOptionURL(forUserId);
  const fetchWebAuthnOptions = async (): Promise<
    PublicKeyCredentialCreationOptionsJSON | WebAuthnRegistrationErrorMessage
  > => {
    const response: PublicKeyCredentialCreationOptionsJSON | WebAuthnRegistrationErrorMessage =
      await fetch(webAuthnOptionURL, {
        headers: {
          Authorization: `${withAuthToken}`,
        },
...
export const useStartWebAuthnRegistration = async (
  withTheseOptions: PublicKeyCredentialCreationOptionsJSON,
): Promise<RegistrationResponseJSON> => await startRegistration(withTheseOptions);

// 3. Send the credential to the server to be verified
export const useVerifyAttestationResult = async (
  forUserId: string,
  withThisResult: RegistrationResponseJSON,
...
export const useWebAuthnRegistration = (
  ...
  const fetchWebAuthnOptions = useFetchWebAuthnOptions(forUserId, withAuthToken);
  ...
  const register = async (): Promise<boolean> => {
    ...
    const verified = await verifyAttestationResult(forUserId, result, withAuthToken);
    if (!verified) {
...
// 1. Create options
export const useFetchWebAuthnLoginOptions = (
  forUsername: string,
): (() => Promise<{
...
export const useStartAuthentication = async (
  withTheseOptions: PublicKeyCredentialRequestOptionsJSON,
) => await startAuthentication(withTheseOptions);

// 3. Send the assertion to the server to be verified
export const useVerifyAssertionResult = async (
  withThisResult: AuthenticationResponseJSON,
  withThisId: string,
): Promise<boolean> =>
